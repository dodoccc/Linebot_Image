...
def linebot():
    ...
    try:
            ...
            if re.match('去除背景',msg):
                if pic == True:                              # 有可處裡圖片                   
                    ...
                    image_message = ImageSendMessage(                 # 放做好的圖片
                        ...
                        original_content_url = pic_link,             
                        # 載入時顯示的圖片
                        preview_image_url = 'https://png.pngtree.com/png-vector/20211111/ourmid/pngtree-minimal-loading-icon-graphic-png-image_4029241.png'
                    )
                    line_bot_api.reply_message(reply_token, image_message)
                    print('圖片已傳送')   
                    pic = False
        ...
        #黑白
                '''
            elif re.match('變成黑白',msg):
                if pic == True:
                    reply = '圖片處理中...'
                    black_white(imgmsg)
                    pic_link = upload_picture(f'BW_{imgmsg}.png',imgur_id,imgur_secret)
                    image_message = ImageSendMessage(                 # 放做好的圖片
                        # 完全跑完的圖片
                        original_content_url = pic_link,             
                        # 載入時顯示的圖片
                    ...
                    reply_text = False   
                    pic = False
                    #os.remove(f'BW_{imgmsg}.jpg') #刪掉做好的圖檔 
                else:
                    reply = '未傳送圖片或是此圖片已處理完成'
                ''' 
        #模糊
            elif re.match('模糊圖片',msg):
                if pic == True:
                    reply = '模糊圖片中...'   
                    blurring(imgmsg)
