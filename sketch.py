...
def sketch(imgmsg):
    img = Image.open(f'{imgmsg}') #打開图片
    new = Image.new("L", img.size, 255) #新文件，大小和原来文件一样
    width, height = img.size #图片大小
    img = img.convert("L")

    Pen_size = 3 #int(input('Pensize:')) #画笔大小
    Color_diff = 3 #int(input('Color diffusion variable:')) #色差扩散器
